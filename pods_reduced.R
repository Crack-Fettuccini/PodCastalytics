...
library(tuneR)
library(R.utils)
#to include python local dependency
...
for (pagelink in pagelinks){
  ...
  for (i in 1:net_size){
    dataliner[i]=str_extract(
                    str_extract(datalines[i],'url="https(.*?).mp3'),
                                                'https(.*?).mp3')  
  }
  
  csvfile<-paste0("./ScrappedData/pod_lean", linkcount, ".csv")
  dfsetup<-data.frame("url","Neutral_bar","Happy_bar","Sad_bar","Anger_bar","Fear_bar","time")
  write.csv(dfsetup, csvfile)
  ...
  for (url in dataliner){
    ...
    write.table(row2, file = csvfile, sep = ",",
